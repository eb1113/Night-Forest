...
void TileMap::generateGrid(int width, int depth, float tileSize) {
    ...
    float scale = 0.02f;      // controls frequency of terrain
    float amplitude = 10.0f;  // max height of terrain
...
void TileMap::generateFoliage(int seed){
    if (foliageGenerated) return;
    
    trees.clear();
    shrubs.clear();
    
    // Lambda to capture 'this' for getHeightAt
    auto heightFunc = [this](float x, float z) -> float {
        return this->getHeightAt(x, z);
    };
    
    // Generate clusters
    auto clusters = FoliageGenerator::generateClusters(gridWidth, gridDepth, tileSize, seed);
    
    // Generate vegetation for each cluster
    for (const auto& cluster : clusters) {
        if (cluster.vegetationType == 0 || cluster.vegetationType == 2) {
            // Generate trees
            auto clusterTrees = FoliageGenerator::generateTrees(
                cluster, heightFunc, seed);
            trees.insert(trees.end(), clusterTrees.begin(), clusterTrees.end());
        }

        if(cluster.vegetationType == 1 || cluster.vegetationType == 2) {
            // Generate shrubs
            auto clusterShrubs = FoliageGenerator::generateShrubs(
                cluster, heightFunc, seed);
